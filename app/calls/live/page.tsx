...
import { Sparkles, Play, Mic, StopCircle, Activity, TrendingUp, ArrowRight } from 'lucide-react';
...
function LiveCallContent() {
  ...
  const repRecords = records.filter(
    (r) => r.speaker?.toLowerCase().includes('seller') || r.speaker?.toLowerCase().includes('you')
  );
  const totalWithSpeaker = records.filter((r) => r.speaker).length;
  const repRatio =
    totalWithSpeaker > 0 ? Math.round((repRecords.length / totalWithSpeaker) * 100) : 45;
            ...
            <div className="bg-white rounded-3xl p-8 max-w-sm w-full text-center space-y-6 border border-slate-100 shadow-2xl">
              ...
              <div className="space-y-2">
                ...
                <p className="text-xs text-slate-500 leading-relaxed">
                  Generating LLM executive summaries, mapping opportunity checklists, and writing
                  back to database.
                </p>
              </div>
            </div>
                ...
                <div className="flex items-center gap-3 pb-4 border-b border-slate-100">
                  ...
                  <div>
                    ...
                    <p className="text-[10px] text-[#6B7280]">
                      Configure your streaming session to start real-time coaching
                    </p>
                  </div>
                </div>
          ...
          <div className="h-full flex flex-col justify-between overflow-hidden">
                ...
                <div className="border-t border-slate-100 pt-4 mt-4 shrink-0 space-y-2">
                  ...
                  <div className="h-14 bg-slate-50 border border-slate-100 rounded-xl flex items-end p-2 gap-1 overflow-x-auto">
                    {records.length > 0 ? (
                      ...
                      <div className="w-full text-center text-[10px] text-slate-400 font-semibold uppercase py-3">
                        No data points yet
                      </div>
                    )}
                  </div>
                ...
                <div className="bg-[#4F46E5] text-white rounded-2xl p-6 shadow-xl space-y-4 relative overflow-hidden shrink-0">
                  ...
                  <div className="space-y-2">
                    ...
                    <p className="text-sm font-semibold leading-relaxed">
                      {latestRecommendation ||
                        'AI is evaluating conversation to generate next tactic recommendations...'}
                    </p>
                  </div>
                </div>
                ...
                <div className="bg-white border border-[#E5E7EB] rounded-2xl p-6 shadow-sm space-y-4 shrink-0">
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    {[
                      ...
                      <div
                        key={i}
                        className="border border-slate-100 bg-slate-50/50 rounded-xl p-3 space-y-1.5"
                      >
                        <div className="flex items-center justify-between">
                          ...
                          <span
                            className={`w-2.5 h-2.5 rounded-full ${item.met ? 'bg-[#10B981]' : 'bg-slate-200'}`}
                          />
                        </div>
                    ...
                    {allHesitations.length > 0 ? (
                      <div className="flex flex-wrap gap-2">
                        {allHesitations.map((h, i) => (
                          <span
                            key={i}
                            className="text-[10px] bg-red-50 text-red-700 px-2 py-0.5 rounded font-bold border border-red-100"
                          >
                            {h}
                          </span>
                        ))}
                      ...
                      <p className="text-[10px] text-slate-400 font-semibold uppercase">
                        No risk objects detected in transcript
                      </p>
                    )}
                  </div>
                </div>
            ...
            <div className="h-20 shrink-0 bg-white border-t border-[#E5E7EB] px-6 flex items-center justify-between gap-6">
              <div className="flex items-center gap-4">
                <div
                  className={`w-10 h-10 rounded-full flex items-center justify-center ${isRecording ? 'bg-red-50 text-red-600' : 'bg-slate-50 text-slate-500'}`}
                >
                  <Mic className={`w-5 h-5 ${isRecording ? 'animate-pulse' : ''}`} />
                </div>
                <div>
                  <span className="text-[10px] text-slate-400 font-bold uppercase block">
                    Streaming target client
                  </span>
