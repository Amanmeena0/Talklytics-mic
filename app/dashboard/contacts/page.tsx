...
export default function ContactsPage() {
      ...
      try {
        ...
        if (res.ok) {
          ...
          const contactsList = Object.values(contactsMap).map((contact: any) => {
            ...
            callList.sort((a: any, b: any) => new Date(b.date).getTime() - new Date(a.date).getTime());
            
            const totalCalls = callList.length;
            const latestCall = callList[0];
            
            // Compute average win probability & sentiment
            const avgProb = Math.round(callList.reduce((acc: number, c: any) => acc + (c.conversionProbability || 50), 0) / totalCalls);
            ...
            const totalBantChecks = (bantBudgetCount + bantAuthorityCount + bantNeedCount + bantTimelineCount) / (totalCalls * 4);
  ...
  const filteredContacts = contacts.filter((c) => {
    ...
    const matchesSentiment = 
      sentimentFilter === 'All' || 
      c.avgSentiment === sentimentFilter;
        ...
        <div className="max-w-7xl mx-auto space-y-8">
          ...
          {isLoading ? (
            <div className="bg-white border border-[#E5E7EB] rounded-3xl shadow-sm p-8 space-y-4">
              {[1, 2, 3].map((n) => (
                <div key={n} className="flex items-center justify-between animate-pulse pb-4 border-b border-slate-100 last:border-0 last:pb-0">
            ...
            <div className="bg-white border border-[#E5E7EB] rounded-3xl p-16 text-center space-y-4 shadow-sm">
              ...
              <div className="space-y-1">
                <h3 className="text-sm font-bold text-slate-800 uppercase tracking-wider">No Contacts Found</h3>
                ...
                <table className="w-full text-left border-collapse">
                  ...
                  <tbody className="divide-y divide-slate-100 text-xs">
                    {filteredContacts.map((contact) => (
                      <tr key={contact.company} className="hover:bg-slate-50/40 transition-colors group">
                        <td className="py-4 px-6">
                          <div className="flex items-center gap-3">
                            ...
                            <div>
                              <span className="font-bold text-slate-900 block">{contact.company}</span>
                        ...
                        <td className="py-4 px-6">
                          <div>
                            <span className="text-slate-800 font-semibold block truncate max-w-xs">{contact.latestTitle}</span>
                            <span className="text-[10px] text-slate-400 flex items-center gap-1 mt-0.5">
                              <Calendar className="w-3 h-3" />
                              {new Date(contact.latestDate).toLocaleDateString(undefined, { dateStyle: 'medium' })}
                            </span>
                          </div>
                        </td>
                        ...
                        <td className="py-4 px-6 text-center">
                          <div className="flex items-center justify-center gap-2">
                            ...
                            <span className="font-bold text-slate-800 text-[11px]">{contact.avgProbability}%</span>
                          </div>
                        </td>
                        ...
                        <td className="py-4 px-6">
                          <div className="flex items-center gap-2">
                            <div className="w-6 h-6 rounded-full overflow-hidden bg-slate-100 border border-slate-200">
                              <img src={contact.ownerAvatar} className="w-full h-full object-cover" alt="Owner" />
                            </div>
                            <span className="font-semibold text-slate-700 text-[11px]">{contact.owner}</span>
                          </div>
                        </td>
