...
export default function TranscriptLog() {
  ...
  // Get unique speakers
  const speakers = hasData
    ? Array.from(new Set(records.map(r => r.speaker)))
    : [];
    ...
    <section className="card-flush">
      ...
      >
        <div style={{ display: 'flex', alignItems: 'center', gap: 'var(--space-4)' }}>
          {/* Main Tab Switcher */}
          <div style={{ display: 'flex', gap: 'var(--space-1)', background: 'var(--bg-root)', padding: '4px', borderRadius: 'var(--radius-md)' }}>
            <button
            ...
            {!isLive && (
              ...
              >
                Comments
                {comments.length > 0 && (
                  <span style={{ background: 'var(--accent)', color: 'white', fontSize: '10px', padding: '1px 6px', borderRadius: '50%' }}>
                    {comments.length}
                  </span>
                )}
              </button>
      ...
      >
        {activeTab === 'transcript' ? (
          ...
          hasData ? (
              ...
              {filteredRecords.length > 0 ? (
                filteredRecords.map((record, i) => {
                  ...
                  const initials = record.speaker
                    ? record.speaker.split(' ').map(w => w[0]).join('').slice(0, 2)
                    : '?';

                  return (
                    <div key={record.id || i} className="transcript-entry" style={{ animation: 'fadeInUp 0.3s ease' }}>
                        ...
                        <div style={{ display: 'flex', alignItems: 'center', gap: 'var(--space-2)', marginBottom: 'var(--space-1)', flexWrap: 'wrap' }}>
                          ...
                          <span
                            style={{
                              ...
                              background:
                                record.sentiment === 'Positive' ? 'var(--success-muted)' :
                                record.sentiment === 'Negative' ? 'var(--error-muted)' :
                                'rgba(156, 163, 175, 0.1)',
                              color:
                                record.sentiment === 'Positive' ? 'var(--success)' :
                                record.sentiment === 'Negative' ? 'var(--error)' :
                                'var(--text-secondary)',
                            }}
                          >
                            {record.sentiment}
            ...
            <div style={{ textAlign: 'center', padding: 'var(--space-8) 0', color: 'var(--text-muted)' }}>
              <span className="material-symbols-outlined" style={{ fontSize: 48, display: 'block', marginBottom: 'var(--space-3)', opacity: 0.4 }}>mic</span>
              <p style={{ fontSize: 14, fontWeight: 500, margin: 0 }}>
                {status === 'connecting' ? 'Connecting to backend...' : status === 'connected' ? 'Listening for speech...' : 'Waiting for connection'}
              </p>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 'var(--space-6)', height: '100%' }}>
            {/* Comment Form */}
            <form onSubmit={handlePostComment} style={{ display: 'flex', flexDirection: 'column', gap: 'var(--space-3)', background: 'var(--bg-elevated)', padding: 'var(--space-4)', borderRadius: 'var(--radius-md)', border: '1px solid var(--border-default)' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 'var(--space-3)' }}>
                <span className="text-section-heading" style={{ fontSize: '13px', margin: 0 }}>Add Review Note</span>
                {commentTimestamp !== null && (
                  <span style={{ fontSize: '11px', background: 'var(--accent-muted)', color: 'var(--accent)', padding: '2px 8px', borderRadius: '4px', display: 'flex', alignItems: 'center', gap: '4px' }}>
                    Linked to segment {formatTime(commentTimestamp)}
                    <button
                      type="button"
                      onClick={() => setCommentTimestamp(null)}
                      style={{ background: 'none', border: 'none', color: 'var(--accent)', cursor: 'pointer', display: 'flex', padding: 0 }}
                    >
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 'var(--space-4)' }}>
              {comments.length > 0 ? (
                comments.map((comment: any) => (
                  <div key={comment.id} style={{ display: 'flex', gap: 'var(--space-3)', animation: 'fadeInUp 0.3s ease' }}>
                    <div style={{
                      width: '32px',
                      height: '32px',
                      borderRadius: '50%',
                      ...
                      <img
                        src={comment.author?.avatarUrl || 'https://images.unsplash.com/photo-1535713875002-d1d0cf377fde?w=100'}
                        alt={comment.author?.name}
                        style={{ width: '100%', height: '100%', objectFit: 'cover' }}
                      />
                    </div>
                    <div style={{ flex: 1, background: 'var(--bg-elevated)', padding: 'var(--space-3) var(--space-4)', borderRadius: 'var(--radius-md)', border: '1px solid var(--border-default)' }}>
                      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 'var(--space-1)' }}>
                        <span style={{ fontWeight: 600, fontSize: '13px' }}>{comment.author?.name}</span>
                      ...
                      <p className="text-body" style={{ margin: 0, fontSize: '13px', color: 'var(--text-primary)' }}>
                        {comment.content}
                      </p>
                    </div>
                  </div>
